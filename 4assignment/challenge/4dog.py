...
def compute_ranks2(S):
    indexed_S = [(x, i) for i, x in enumerate(S)]
    
    sorted_S = sorted(indexed_S, key=lambda t: (t[0]))
    
    ranks = [sorted_S.index((x, i)) + 1 for i, x in enumerate(S)]
